...
function renderBrief(){
  ...
  if(!scopeIsTeam()){
    ...
    if(!active.length){setBrief(upcoming,'next 7 days',`${name} has no scheduled activity today.`,`${upcoming} appointment${upcoming===1?' is':'s are'} scheduled across the next seven days.`);return}
    if(attention){setBrief(attention,'open outcomes',`${attention} recent outcome${attention===1?' needs':'s need'} a next step.`,`${name}’s activity is isolated in this view.`);return}
    if(atRisk.length){setBrief(`${data.score}%`,'today',`${name} is behind today’s pace.`,'Start with the clearest activity gap in the next coaching conversation.');return}
    setBrief(`${data.score}%`,'today',`${name} is moving with no urgent gaps.`,`${upcoming} appointment${upcoming===1?' is':'s are'} scheduled across the next seven days.`);return
  }
  if(!active.length){setBrief(upcoming,'next 7 days',scopeType()==='all'?'No scheduled activity across the portfolio today.':'No scheduled team activity today.',`${upcoming} appointment${upcoming===1?' is':'s are'} scheduled across the next seven days.`);return}
  if(attention){setBrief(attention,'open outcomes',`${attention} recent outcome${attention===1?' needs':'s need'} a next step.`,atRisk.length?`${atRisk.length} active agent${atRisk.length===1?' is':'s are'} also below 50% today.`:'Close the result or set the next follow-up.');return}
  if(atRisk.length){setBrief(atRisk.length,'below pace',`${atRisk.length} agent${atRisk.length===1?' needs':'s need'} a pace conversation.`,`Use the team pulse below to start with the clearest activity gap.`);return}
  setBrief(`${data.score}%`,'today',scopeType()==='all'?'The portfolio is moving with no urgent gaps.':'The team is moving with no urgent gaps.',`${upcoming} appointment${upcoming===1?' is':'s are'} scheduled across the next seven days.`);
}
...
function renderAgentPulse(){
  const ranked=scopedMembers().map(member=>{const live=state.leaderboard.find(item=>item.uid===member.uid),entry=live||leaderboardFor(member.uid),current=Boolean(live&&entry.date===today()),scheduled=entry.activeToday!==false,score=current?clamp(entry.score):0,priority=current&&scheduled?score:current?180:150;return{member,live,entry,current,score,priority,status:current?agentStatus(entry):'waiting'}}).sort((a,b)=>a.priority-b.priority||memberName(a.member.uid).localeCompare(memberName(b.member.uid))).slice(0,5);
  const rows=ranked.map(({member,live,entry,current,score,status})=>{const meta=!live?'Waiting for current data':entry.activeToday===false?'Not scheduled':current?`${number(entry.calls)} calls · ${number(entry.connects)} connects`:'No activity published today';return`<article class="agent-row ${status}"><div class="agent-name"><strong>${esc(memberName(member.uid))}</strong><small>${esc(meta)}</small></div><div class="progress"><i style="width:${score}%"></i></div><div class="agent-score">${current?`${score}%`:'—'}</div></article>`}).join('');
