...
function renderBrief(){
  ...
  const health=periodAgentHealth(),offTrack=health.filter(item=>item.status==='off-track'),atRisk=health.filter(item=>item.status==='at-risk'),onTrack=health.filter(item=>item.status==='on-track'),missing=health.filter(item=>item.status==='no-data'),attention=periodAttentionAppointments().length,upcoming=periodUpcomingAppointments().length,label=periodLabel();
  if(!scopeIsTeam()){
  ...
  const context=[atRisk.length?`${atRisk.length} at risk`:'',attention?`${attention} outcome${attention===1?'':'s'} open`:'',missing.length?`${missing.length} with no data`:''].filter(Boolean).join(' · ')||`${upcoming} upcoming appointment${upcoming===1?' was':'s were'} created ${label}`;
...
function renderTrends(){
  ...
  $('#trendTable').innerHTML=scopedMembers().map(member=>{const entry=leaderboardFor(member.uid),agentData=aggregateEntry(entry),direction=directionFor(entry),connect=agentData.calls?Math.round(agentData.connects/agentData.calls*100):0;return`<tr><td>${esc(memberName(member.uid))}</td><td>${agentData.scheduledDays}</td><td>${agentData.score}%</td><td>${agentData.calls}</td><td>${connect}%</td><td>${agentData.appointments}</td><td class="${direction.className}">${esc(direction.label)}</td></tr>`}).join('');
}
